...
## 설치
* [Releases 페이지](https://github.com/Aniana0/dvcfg_pytools/releases)나 [여기(최신 버전)](https://github.com/Aniana0/dvcfg_pytools/releases/download/1.1.0/dvcfg_pytools-1.1.0.tar.gz)에서 배포 파일을 다운로드합니다.
* 이후 다운로드한 파일이 있는 위치에서 아래와 같이 명령어를 실행합니다.
```
pip install dvcfg_pytools-(버전).tar.gz
```
또는
```
python -m pip install dvcfg_pytools-(버전).tar.gz
```
...
**이 모듈에서 마커의 시간 단위는 모두 초(s)입니다.**   
   
마커가 위치한 시간은   
   * CV 타입 : 0.06초 < CP < PP < VSP < VEP
   * V_X, INDIE 타입 : 0.06초 < SP < EP   

가 되어야 하며, 각각의 마커가 위치한 시간이 서로 겹치면 안 됩니다.

## 클래스
* ### Dvcfg(파일이름)
   * dvcfg 객체를 만드는 클래스입니다.
   * 객체 생성 시 voice.dvcfg 파일 이름을 입력하면 자동으로 voice.dvcfg 데이터를 읽어들입니다.
...
* #### load( 파일이름='voice.dvcfg' )
   * voice.dvcfg 파일에서 데이터를 읽어들입니다.
   * 아무것도 입력하지 않으면 작업중인 디렉토리의 'voice.dvcfg'를 읽어들입니다.
...
* #### markers_CV( 발음기호 , 피치, 모드='dict' )
   * CV 타입인 항목의 마커가 위치한 시간을 반환합니다.
   * 모드는 'dict'와 'tuple' 두 가지가 있습니다.   
      * 'dict' - 마커가 위치한 시간을 딕셔너리로 반환합니다.
      * 'tuple' - 마커가 위치한 시간을 다음과 같은 순서로 튜플로 반환합니다. ( CP , PP , VSP , VEP )
   * 모드에 아무것도 입력하지 않으면, 자동으로 딕셔너리로 반환합니다.

* #### markers_VX( 발음기호 , 피치, 모드='dict' )
   * VX 타입인 항목의 마커가 위치한 시간을 반환합니다.
   * 모드는 'dict'와 'tuple' 두 가지가 있습니다.   
      * 'dict' - 마커가 위치한 시간을 딕셔너리로 반환합니다.
      * 'tuple' - 마커가 위치한 시간을 다음과 같은 순서로 튜플로 반환합니다. ( SP , EP )
   * 모드에 아무것도 입력하지 않으면, 자동으로 딕셔너리로 반환합니다.

* #### markers_INDIE( 발음기호 , 피치, 모드='dict' )
   * INDIE 타입인 항목의 마커가 위치한 시간을 반환합니다.
   * 모드는 'dict'와 'tuple' 두 가지가 있습니다.   
      * 'dict' - 마커가 위치한 시간을 딕셔너리로 반환합니다.
      * 'tuple' - 마커가 위치한 시간을 다음과 같은 순서로 튜플로 반환합니다. ( SP , EP )
   * 모드에 아무것도 입력하지 않으면, 자동으로 딕셔너리로 반환합니다.
...
* #### edit_CV( 발음기호 , 피치 , CP , PP , VSP , VEP )
   ...
   * 마커값에 0을 입력할 경우 기존의 마커가 위치한 시간을 유지합니다.
